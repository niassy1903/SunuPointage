<?php

namespace App\Models;

use MongoDB\Laravel\Eloquent\Model;
use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\Hash;
use Illuminate\Notifications\Notifiable;
use Illuminate\Contracts\Auth\CanResetPassword;
use Illuminate\Auth\Passwords\CanResetPassword as CanResetPasswordTrait;
use Illuminate\Database\Eloquent\SoftDeletes;

class Utilisateur extends Model implements CanResetPassword
{
    use Notifiable, CanResetPasswordTrait, SoftDeletes;

    // Connexion à MongoDB
    protected $connection = 'mongodb';
    protected $collection = 'utilisateurs';

    // Attributs remplissables
    protected $fillable = [
        'nom',
        'prenom',
        'email',
        'adresse',
        'telephone',
        'fonction',
        'photo',
        'mot_de_passe',
        'date_creation',
        'date_suppression',
        'date_modification',
        'departement',
        'cohorte',
        'matricule',
        'status',
        'card_id',
    ];

    // Cast des dates
    protected $casts = [
        'date_creation' => 'datetime',
        'date_suppression' => 'datetime',
        'date_modification' => 'datetime',
    ];

    // Méthode nécessaire pour la réinitialisation du mot de passe
    public function getEmailForPasswordReset()
    {
        return $this->email;
    }

    // Actions lors de la création, mise à jour et suppression
    protected static function boot()
    {
        parent::boot();

        static::creating(function ($model) {
            $model->date_creation = Carbon::now();
            $model->status = 'actif'; 
            $model->matricule = self::generateMatricule(); // Génération automatique du matricule
        });

        static::updating(function ($model) {
            $model->date_modification = Carbon::now();
        });

        static::deleting(function ($model) {
            $model->date_suppression = Carbon::now();
            $model->save();
        });
    }

    protected static function generateMatricule()
    {
        // Forme : MAT-2025-00001
        $year = now()->year;
        $lastMatricule = self::orderBy('_id', 'desc')->value('matricule');
        $incrementedId = 1;

        if ($lastMatricule) {
            $lastId = (int) str_replace("MAT-{$year}-", '', $lastMatricule);
            $incrementedId = $lastId + 1;
        }

        return "MAT-{$year}-" . str_pad($incrementedId, 5, '0', STR_PAD_LEFT);
    }

    // Setter pour hacher automatiquement le mot de passe avant de l'enregistrer
    public function setMotDePasseAttribute($value)
    {
        $this->attributes['mot_de_passe'] = bcrypt($value);
    }

    // Setter pour la fonction
    public function setFonctionAttribute($value)
    {
        $validFunctions = ['apprenant', 'vigile', 'admin', 'employer'];
        if (!in_array($value, $validFunctions)) {
            throw new \InvalidArgumentException("Invalid function value.");
        }
        $this->attributes['fonction'] = $value;
    }

    // Méthode pour vérifier l'existence d'un téléphone
    public function checkTelephoneExists($telephone)
    {
        return self::where('telephone', $telephone)->exists();
    }

    // Relation avec les pointages
    public function historicPointages()
    {
        return $this->hasMany(HistoricPointage::class, 'utilisateur_id');
    }

    // Accesseur pour l'URL de la photo
    public function getPhotoUrlAttribute()
    {
        if ($this->photo) {
            return asset('storage/' . $this->photo); // URL pour une photo stockée localement
        }
        return null;
    }
}
