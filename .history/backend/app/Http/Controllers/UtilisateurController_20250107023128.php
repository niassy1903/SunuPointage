    ...
    public function getUtilisateurByCardId($cardId)
{
    ...
    public function login(Request $request)
    {
        ...
        // Création du payload pour le JWT (utilisateur et rôles)
        $payload = [
            'id' => $utilisateur->_id,
            'roles' => $utilisateur->roles,
        ];
    
        // Génération du token JWT avec expiration à 1 heure
        $token = JWTAuth::encode($payload, env('JWT_SECRET'), 'HS256', time() + 3600); // 3600 secondes = 1h
        ...
        return response()->json([
            ...
            'token' => $token, // Retourner le token JWT
        ]);
    }
