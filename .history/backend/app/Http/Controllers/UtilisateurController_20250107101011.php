    ...
    public function login(Request $request)
{
    ...
    // Récupérer la clé JWT et afficher dans les logs
    $jwtSecret = env('JWT_SECRET');
    Log::info('Clé JWT utilisée : ' . $jwtSecret);

    // Vérifier que la clé est une chaîne
    if (!is_string($jwtSecret) || empty($jwtSecret)) {
        return response()->json(['message' => 'Clé JWT invalide dans .env'], 500);
    }
