    ...
    public function getUtilisateurByCardId($cardId)
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'fonction' => 'required|string|in:apprenant,vigile,admin,employer',
            'photo' => 'nullable|image|mimes:jpeg,png,jpg|max:2048',
            'mot_de_passe' => 'required|string|min:8',
            'departement' => 'nullable|string|max:255',
            'cohorte' => 'nullable|string|max:255',
            'matricule' => 'nullable|string|max:255|unique:utilisateurs,matricule',
        ]);
        ...
        $utilisateur->fill($request->except('photo', 'mot_de_passe'));
        $utilisateur->photo = $photoPath;
        $utilisateur->mot_de_passe = $request->input('mot_de_passe');
        $utilisateur->save();

        return response()->json(['message' => 'Utilisateur enregistré avec succès', 'data' => $utilisateur], 201);
    }
