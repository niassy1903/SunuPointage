        ...
        while ($debut <= $fin) {
            foreach ($employes as $employe) {
                ...
                if ($statut === Pointage::STATUT_PRESENT || $statut === Pointage::STATUT_RETARD) {
                    ...
                    // Vérification obligatoire des heures
                    if ($heureArrivee && $heureDepart) {
                        $tempsTravail = Carbon::createFromFormat('H:i', $heureDepart)
                            ->diffInMinutes(Carbon::createFromFormat('H:i', $heureArrivee));
                    } else {
                        // Si les heures sont manquantes, éviter les incohérences
                        throw new \Exception("Les statuts présent ou retard nécessitent une heure d'arrivée et une heure de départ.");
                    }
                }
